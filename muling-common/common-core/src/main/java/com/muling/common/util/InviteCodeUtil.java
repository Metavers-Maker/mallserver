...
public class InviteCodeUtil {
     ...
     */
    private final static int PRIME2 = 9;
    ...
    public static String makeCodeByUIDUnique(long id) {
        StringBuilder sb = new StringBuilder();
        Random random = new Random(id); // 将ID作为随机数生成器的种子
        for (int i = 0; i < INVITE_CODE_LENGTH; i++) {
            int randomIndex = random.nextInt(BASE); // 生成[0, BASE)之间的随机整数
            sb.append(CHARACTERS.charAt(randomIndex)); // 取出字符集中对应的字符，拼接到邀请码中
        }
        return sb.toString();
    }
