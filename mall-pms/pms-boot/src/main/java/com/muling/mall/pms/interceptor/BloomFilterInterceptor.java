package com.muling.mall.pms.interceptor;
...
public class BloomFilterInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            pathVariable = matcher.extractUriTemplateVariables("/app-api/v1/spu/{spuId}", currentUrl);
        } catch (IllegalStateException e) {
            // 路径不匹配则放行
            return true;
        }
        if (bloomRedisService.includeByBloomFilter(PmsConstants.GOODS_BLOOM_FILTER, pathVariable.get("spuId"))) {
            return true;
        }
