...
public class SmsCodeTokenGranter extends AbstractTokenGranter {
    ...
    protected OAuth2Authentication getOAuth2Authentication(ClientDetails client, TokenRequest tokenRequest) {
        ...
        String verifyCode = parameters.get("verify_code"); // 短信验证码
        String inviteCode = parameters.get("invite_code"); //邀请码

        parameters.remove("verify_code");
        parameters.remove("invite_code");

        Authentication userAuth = new SmsCodeAuthenticationToken(mobile, verifyCode, inviteCode);
