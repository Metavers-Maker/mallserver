...
import com.muling.auth.security.core.clientdetails.ClientDetailsServiceImpl;
...
import com.muling.auth.security.core.userdetails.user.SysUserDetails;
import com.muling.auth.security.core.userdetails.user.SysUserDetailsServiceImpl;
import com.muling.auth.security.extension.alipay.AlipayTokenGranter;
import com.muling.auth.security.extension.captcha.CaptchaTokenGranter;
import com.muling.auth.security.extension.email.MailCodeTokenGranter;
import com.muling.auth.security.extension.facebook.FacebookTokenGranter;
import com.muling.auth.security.extension.google.GoogleTokenGranter;
import com.muling.auth.security.extension.metemask.MetaMaskTokenGranter;
import com.muling.auth.security.extension.mobile.SmsCodeTokenGranter;
import com.muling.auth.security.extension.refresh.PreAuthenticatedUserDetailsService;
import com.muling.auth.security.extension.username.UsernameTokenGranter;
import com.muling.auth.security.extension.wechat.WechatTokenGranter;
import com.muling.auth.security.extension.wxopen.WxopenTokenGranter;
...
public class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
    ...
    public void configure(AuthorizationServerEndpointsConfigurer endpoints) {
        ...
        // 添加微信开放平台授权模式的授权者
        granterList.add(new WxopenTokenGranter(endpoints.getTokenServices(), endpoints.getClientDetailsService(),
                endpoints.getOAuth2RequestFactory(), authenticationManager
        ));

        // 添加支付宝授权模式的授权者
        granterList.add(new AlipayTokenGranter(endpoints.getTokenServices(), endpoints.getClientDetailsService(),
                endpoints.getOAuth2RequestFactory(), authenticationManager
        ));
        ...
        // 添加password授权模式的授权者
        granterList.add(new UsernameTokenGranter(endpoints.getTokenServices(), endpoints.getClientDetailsService(),
                endpoints.getOAuth2RequestFactory(), authenticationManager
        ));

        // 添加邮箱授权模式的授权者
        granterList.add(new MailCodeTokenGranter(endpoints.getTokenServices(), endpoints.getClientDetailsService(),
                endpoints.getOAuth2RequestFactory(), authenticationManager
        ));

        // 添加metamask授权模式的授权者
        granterList.add(new MetaMaskTokenGranter(endpoints.getTokenServices(), endpoints.getClientDetailsService(),
                endpoints.getOAuth2RequestFactory(), authenticationManager
        ));
        ...
        return (accessToken, authentication) -> {
            ...
            if (userAuthentication != null) {
                ...
                if (principal instanceof SysUserDetails) {
                    ...
                    additionalInfo.put("memberId", sysUserDetails.getUserId());//测试用
                    ...
                    additionalInfo.put("isNew", memberUserDetails.getIsNew());
